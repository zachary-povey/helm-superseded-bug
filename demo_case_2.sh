...
helm install -n helm-ss-bug helm-ss-bug ./chart --set crd_name=$CORRECT_CRD_NAME > /dev/null 2>&1
log "Initial install succeeds:"
helm history -n helm-ss-bug helm-ss-bug
log "\nNow if we break the name and upgrade..."
helm upgrade -n helm-ss-bug helm-ss-bug ./chart --set crd_name=invalid > /dev/null 2>&1
