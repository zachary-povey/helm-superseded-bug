...
helm install -n helm-ss-bug helm-ss-bug ./chart --set crd_name=$INVALID_CRD_NAME > /dev/null 2>&1
...
helm upgrade -n helm-ss-bug helm-ss-bug ./chart --set crd_name=$VALID_CRD_NAME > /dev/null 2>&1
